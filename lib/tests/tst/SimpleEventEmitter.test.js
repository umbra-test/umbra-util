...
describe("SimpleEventEmitter Utilities", () => {
    describe("union", () => {
        it("should return an arrayB if the first array does not exist", () => {
            ...
            for (const value of [undefined, null]) {
                expect(ArrayUtils_1.union(value, arrayB)).to.deep.equal(arrayB);
            }
        });
        it("should return arrayA if arrayB doesn't exist", () => {
            const arrayA = ["thing"];
            for (const value of [undefined, null]) {
                expect(ArrayUtils_1.union(arrayA, value)).to.deep.equal(arrayA);
            }
        });
        it("should return an empty array if both arrayA and arrayB don't exist", () => {
            for (const value of [undefined, null]) {
                expect(ArrayUtils_1.union(value, value)).to.deep.equal([]);
            }
        });
        it("should union arrays of different values", () => {
            const arrayA = ["a", "b", "c"];
            const arrayB = ["d", "e", "f"];
            expect(ArrayUtils_1.union(arrayA, arrayB)).to.deep.equal(["a", "b", "c", "d", "e", "f"]);
        });
        ...
        it("should not recursively go below the top level", () => {
            ...
            expect(ArrayUtils_1.union(arrayA, arrayB)).to.deep.equal([{
                    a: "a",
                    b: "b"
                }, {
                    a: "a",
    ...
    describe("without", () => {
        it("should return an empty array if the array does not exist", () => {
            for (const value of [undefined, null]) {
                expect(ArrayUtils_1.without(value, "a")).to.deep.equal([]);
            }
        });
        it("should return the array unchanged if the expected value doesn't exist", () => {
            const array = ["a", "b", "c"];
            expect(ArrayUtils_1.without(array, "d")).to.deep.equal(array);
        });
        it("should remove elements from the array that exist", () => {
            const array = ["a", "b", "c"];
            expect(ArrayUtils_1.without(array, "b")).to.deep.equal(["a", "c"]);
        });
        it("should NOT REMOVE DUPLICATE VALUES -- this is an important caveat of this implementation", () => {
            const array = ["a", "b", "b", "b", "c"];
            expect(ArrayUtils_1.without(array, "b")).to.deep.equal(["a", "b", "b", "c"]);
        });
    });
});
//# sourceMappingURL=SimpleEventEmitter.test.js.map